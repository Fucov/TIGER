...
def parse_args():
    ...
    parser.add_argument(
        "--lr_scheduler_type", type=str, default="linear", help="scheduler"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="./ckpt/Beauty",
        help="please specify output directory for model",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="weight for InfoNCE contrastive loss (0 to disable)",
    )
    parser.add_argument(
        "--temperature",
        type=float,
...
if __name__ == "__main__":
    ...
    ensure_dir(log_dir)
    log_file = os.path.join(log_dir, f"rqvae_{get_local_time()}.log")
    logging.basicConfig(
        level=logging.INFO,
