    ...
    with torch.no_grad():
        for batch in tqdm(eval_loader, desc="Evaluating"):
            ...
            preds = model.generate(
                input_ids=input_ids, attention_mask=attention_mask, num_beams=beam_size
            )
            preds = preds[:, 1:]  # Exclude the start token
            preds = preds.reshape(
                input_ids.shape[0], beam_size, -1
...
if __name__ == "__main__":
    ...
    for epoch in range(config["num_epochs"]):
        ...
        if avg_ndcgs["NDCG@20"] > best_ndcg:
            ...
            logging.info(
                f"No improvement in NDCG@20. Early stop counter: {early_stop_counter}"
            )
